...
def flash_attn(q, k, v, scale, dropout_p=0.0, causal=False):
    ...
    Args:
        q: Query tensor (batch, seq_len, head_dim) on 'mps' device
        k: Key tensor (batch, seq_len, head_dim) on 'mps' device
        v: Value tensor (batch, seq_len, head_dim) on 'mps' device
